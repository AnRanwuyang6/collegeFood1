    ...
    $(function(){
        ...
        $('.operate_00').attr("class", "nav-item start active open");

        $('.operate_01').attr("class", "nav-item start active open");
    ...
    var initFunction = function () {
        status=$('#typeStatusSelect').val();
        $('#typeStatusSelect').change(function () {
            status = $(this).val();
        /*queryData(opts.current_page + 1,opts.items_per_page);*/
            queryData(1,10);
    });
    ...
    var url = '/afterFoodType/ListAjax'
    $.ajax({
        url: url,
        type: 'post',
        dataType: 'json',
        ...
        success: function(data){
            ...
            if(data.code == '0000'){
                ...
                for (i in content){
                    ...
                    if(content[i].typeStasus == '1'){
                        html += ('上架')
                    }else{
                        html += ('下架')
                    }
                    html += ('<td nowrap="nowrap">'+content[i].remark+'</td>')
                    html += ('</td>')
                    html += ('<td nowrap="nowrap">')
                    if(content[i].typeStasus == '1'){
                    ...
                    html += ('<div class="ui green button" id="addFood" onclick="addFood(\'/foodInfor/html?typeId='+content[i].id+'&typeName='+content[i].typeName+'&flag='+content[i].str1+'\')">新增物品</div>')
                    html += ('</td>')
                    html += '</tr>'
                }
                $('#content').append(html)
  ...
  /*  var editSubmit = function () {
    ...
    var alertFlag=function(id){
        var url = "/afterFoodType/alertFlag/" + id;
        $.ajax({
            url:url,
            type:'post',
            dataType:'json',
