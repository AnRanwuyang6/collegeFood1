...
$(function () {
    ...
    $('.edit_03_03_01').attr("class", "nav-item start active open");
...
var queryData = function (page, size) {
    $.ajax({
        url: "/secondInstance/getWriterPage",
        type: "post",
        data: {
            "page": page,
            "size": size,
        ...
        success: function (data) {
            if (data.code == "0000") {
                ...
                for (i in n) {
                    ...
                    html += ('<td nowrap="nowrap"><div class="ui button" onclick="toInfo(\'' + n[i].id + '\');">查看</div></td>');
                    html += ('<td nowrap="nowrap"><div class="ui button" onclick="toDelete(\'' + n[i].id + '\', \'' + n[i].manuUserName + '\');">删除</div></td>');
                    html += ('<td nowrap="nowrap"><div class="ui button" onclick="toResetPWD(\'' + n[i].manuUserName + '\');">重置密码</div></td>');
                    html += '</tr>'
                }
                $("#content").append(html);
            } else {
...
var resetPWD = function () {
    $.ajax({
        url: "/secondInstance/resetWriterPWD",
        type: "post",
        data: {
...
var toInfo = function (id) {
    $.ajax({
        url: "/secondInstance/selectWriterInfo",
        type: "post",
        data: {
            "id": id
        },
...
var toDelete = function (id, userName) {
    $("#deleteUserModal").modal('setting', 'closable', false).modal('show');
    $("#hidDeleteUserId").val(id);
    $("#hidDeleteWriterUserName").val(userName);
};

var deleteUser = function () {
    $("#deleteUserSubmit").addClass("disabled");
    $.ajax({
        url: "/secondInstance/deleteWriter",
        type: "post",
        data: {
            "id": $("#hidDeleteUserId").val(),
            "userName": $("#hidDeleteWriterUserName").val()
        },
        success: function (data) {
            if (data.code == "0000") {
                cautionModal("删除成功");
                queryData(opts.current_page + 1, opts.items_per_page);
                $("#deleteUserSubmit").removeClass("disabled")
            } else {
                cautionModal(data.message);
                $("#deleteUserSubmit").removeClass("disabled")
            }
