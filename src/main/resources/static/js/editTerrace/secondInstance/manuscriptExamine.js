...
var queryData = function (page, size) {
    $.ajax({
        url: "/secondInstance/getExaminePage",
        type: "post",
        data: {
            "page": page,
            "size": size,
...
var toDetail = function (id) {
    window.location.href = "/secondInstance/toExamineDetail?id=" + id
};
...
var exportDetail = function () {
    if ($("#selectTime").val() != 2) {
        ...
        url = "/secondInstance/exportDetail?";
        $.each(postData, function (i, item) {
            if (item != null && item != "") {
                url += "&" + i + "=" + item;
            }
