...
$(function() {
    ...
    $("#appointUserName").change(function(){
        queryData(opts.current_page + 1, opts.items_per_page);
    })

    $("#manuStatus").change(function(){
        queryData(opts.current_page + 1, opts.items_per_page);
    })
...
var queryData = function(page,size){
    $.ajax({
        ...
        data:{
            ...
            "start":$("#beginTime").val(),
            "end":$("#endTime").val(),
            "condition":$.trim($("#manuName").val()),
            "appointUserName":$("#appointUserName").val(),
            "manuStatus":$("#manuStatus").val()
        },
        success:function(data){
            if (data.code == "0000"){
                ...
                for (i in n) {
                    ...
                    html += ('<td >'+ n[i].manuAppointUser +'</td>')
                    html += ('<td>')
                    if (n[i].manuStatus == '0') {
                        html += ('待提交')
                    ...
                    html += ('<td nowrap="nowrap"><input type="button" class="ui button" value="查看" onclick="toDetail(\''+n[i].id+'\')" /></td>')
                    html += '</tr>'
                }
                $("#content").append(html);
            }else{
...
var toDetail = function(id){
    window.location.href = "/lastInstance/detail?id="+id;
}
...
var exportAll = function () {
    if ($("#selectTime").val() != 2) {
        ...
        postData["condition"] = $("#manuName").val();

        url = "/lastInstance/exportManuscriptAll?";
...
var exportDetail = function () {
    if ($("#selectTime").val() != 2) {
        ...
        url = "/lastInstance/exportManuscriptDetail?";
        $.each(postData, function (i, item) {
            if (item != null && item != "") {
                url += "&" + i + "=" + item;
            }
