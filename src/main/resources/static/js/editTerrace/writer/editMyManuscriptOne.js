...
var add_save = function () {
    ...
    var isIndustry = $("#hidddenIsIndustryId").val()
    var processName = '';
    if(isIndustry == 1){
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.code=='0000'){
                ...
                $('#add_title').val(content.title);
                $('#countWordsId').text(content.wordCount);
                ue.ready(function(){
                    ue.setContent(content.content);
                });
                if (content.urls.length!=0) {
                    for (var i = 0; i < content.urls.length; i++) {
                        $('#tBId tr').eq(i).find('td').eq(0).find('input').val(content.urls[i]);
                        if (i+1 < content.urls.length) {
                            toAddUrl();
                        }
                    }
                }
